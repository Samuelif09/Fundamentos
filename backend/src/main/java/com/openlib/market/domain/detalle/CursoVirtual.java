...
public class CursoVirtual extends ContenidoDigital {
    ...
    public CursoVirtual pausar() {
        ...
        if (getEstado() == EstadoLibro.PAUSADO) {
            throw new com.openlib.market.domain.shared.AccionNoPermitidaException("El curso ya se encuentra pausado");
        }
        if (getEstado() != EstadoLibro.PUBLICADO) {
            throw new TransicionEstadoInvalidaException("Solo los cursos PUBLICADOS pueden ser pausados");
        }
    ...
    public CursoVirtual rechazar(com.openlib.market.domain.curaduria.MotivoRechazo motivo) {
        ...
        if (getEstado() == EstadoLibro.RECHAZADO) {
            throw new com.openlib.market.domain.shared.AccionNoPermitidaException("El curso ya se encuentra rechazado");
        }
        if (getEstado() != EstadoLibro.PENDIENTE) {
